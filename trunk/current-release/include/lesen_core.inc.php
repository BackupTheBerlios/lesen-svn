...
require_once('bundled-libs/DB/DB.php');
...
class Lesen_person
{
  ...
  function loadDB()
  {
    ...
    $query = $db->prepare("select person_lastname lastname, person_name name,
                           person_mail mail, person_webpage webpage
                           from person
                           where code = '?'");
    $result =& $db->execute($query, $this->_code);
    if (PEAR::isError($db)) {
      die($db->getMessage());
    }
    ...
    $query = $db->prepare("select code_paper 
                           from authoring
                           where code_author = '?'");
    $result =& $db->execute($query, $this->_code);
    if (PEAR::isError($db)) {
    ...
    $query = $db->prepare("select code_institution
                           from membership
                           where code_person = '?'");
    $result =& $db->execute($query, $this->_code);
    if (PEAR::isError($db)) 
...
class Lesen_paper
{
  ...
  function loadDB()
  {
    ...
    $query = $db->prepare("select paper_title title, course.course_name course, 
                                  des_spanish, des_english, des_portuguese 
                           from paper, course 
                           where paper.code = ? and
    ...
    /* paper keywords */
    $query = $db->prepare("select k.keyword_spanish spanish, k.keyword_english english
    ...
    $query = $db->prepare("select a.code_author code, r.rol_name rol
                           from authoring a, rol r
	                   where a.code_paper = ? and r.code = a.author_type
